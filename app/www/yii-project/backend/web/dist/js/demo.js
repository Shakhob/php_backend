...
(function ($) {
  ...
  $(document).on('click', '.remove_img', function(e) {
    let userAnswer = confirm('Вы уверены, что хотите удалить этот элемент?');
    if (!userAnswer){
      e.preventDefault();
      return false;
    }
  ...
  $(document).ready(function() {
    // Скрываем кнопку удаления при загрузке страницы
    ...
    $('td input[type="checkbox"]').change(function() {
      ...
      if ($('td input[type="checkbox"]:checked').length > 0) {
        // Делаем кнопку удаления кликабельной
        $('.delete-multiple').prop('disabled', false);
      } else {
        // Делаем кнопку удаления не кликабельной
        $('.delete-multiple').prop('disabled', true);
      }
    });
  });
  ...
  if (previewImages){
    // alert("say hello")
    previewImages.forEach(function (v,i,a) {
      let blah = v.parentNode.parentNode.querySelector('.preview-image img')
